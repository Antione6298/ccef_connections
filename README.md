...
- **Render**: The platform our internal Flask tools run *on* — services, deploys and their status, custom domains and DNS verification, and environment variables. Pass `blueprint_path` and it refuses to write env vars that `render.yaml` owns, which Render would silently revert on the next Blueprint sync
